defmodule Ourocode.Runtime.Stream.Transport do
  @moduledoc """
  OTP process boundary for one MCP transport stream.

  The process stores local stream identity and cursor state only. External
  runtime IDs and statuses remain trusted at their source; ourocode keeps the
  mapping needed by panes, journal recovery, and cleanup supervision.
  """
  ...
  @type state :: %{
          required(:stream_kind) => :transport,
  ...
  def init(opts) do
    state =
      %{
        ...
        stream_cursor: Keyword.get(opts, :stream_cursor, %{}),
        event_count: 0
      }
  ...
  defp next_cursor(state, event) do
    state.stream_cursor
    |> Map.put(:transport, Map.get(event, :transport, state.transport))
  ...
  defp default_child_id(opts) do
    {__MODULE__, Keyword.get(opts, :parent_call_id), Keyword.get(opts, :transport)}
  end
