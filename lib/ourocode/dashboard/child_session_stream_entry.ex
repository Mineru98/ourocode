defmodule Ourocode.Dashboard.ChildSessionStreamEntry do
  @moduledoc """
  Normalizes child-session lifecycle events into renderable stream entries.
  """

  alias Ourocode.Journal
  alias Ourocode.MCP.ChildSessionCreationParser

  @spec entries_for_event(map(), integer(), integer()) :: [map()]
  def entries_for_event(event, event_seq, occurred_at_ms) when is_map(event) do
  ...
  def entries_for_event(_event, _event_seq, _occurred_at_ms), do: []
