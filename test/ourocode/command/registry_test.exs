defmodule Ourocode.Command.RegistryTest do
  ...
  test "loads builtin ourocode commands into a normalized entry set" do
    ...
    assert Enum.map(registry.ordered, & &1.slash) == expected_commands
    assert Map.keys(registry.entries) |> Enum.sort() == Enum.sort(expected_commands)

    assert Enum.all?(registry.ordered, fn entry ->
             entry.source == :builtin and
               entry.source_id == "builtin" and
               entry.type == :slash_command and
               entry.availability in [:available, :stub] and
               ...
               is_map(entry.run_spec)
           end)

    assert Enum.filter(registry.ordered, &(&1.availability == :stub))
           |> Enum.map(& &1.slash) == ["/mcp", "/sessions", "/config"]
  end

  test "stub slash commands route through the same registry and builtin execution path" do
    {:ok, registry} = Registry.load_builtin()

    routed_full =
      for slash <- ["/clear", "/resume", "/exit", "/quit", "/status", "/plugins"] do
        assert {:ok, entry} = Registry.fetch(registry, slash)
        assert entry.source == :builtin
        assert entry.run_spec.kind == :builtin_action
        assert is_atom(entry.run_spec.action)
        {slash, entry.availability}
      end
    ...
    routed_stubs =
      for slash <- ["/mcp", "/sessions", "/config"] do
        assert {:ok, entry} = Registry.fetch(registry, slash)
        assert entry.source == :builtin
        assert entry.availability == :stub
        assert entry.runnable? == true
        assert entry.run_spec.kind == :builtin_action
        {slash, entry.run_spec.action, entry.summary}
      end

    assert routed_stubs == [
             {"/mcp", :show_mcp, "Show stdio, SSE, and streamable HTTP MCP transport status."},
             {"/sessions", :show_sessions, "Show the parent and child session list."},
             {"/config", :show_config, "Show plugin/runtime config and reload guidance."}
           ]
  end

  test "includes builtin-only source metadata; provider connect reintroduced, account commands excluded" do
    {:ok, registry} = Registry.load_builtin()

    sources = registry.ordered |> Enum.map(& &1.source) |> Enum.uniq()
    command_names = Enum.map(registry.ordered, & &1.name)

    assert sources == [:builtin]
    # `/login` `/logout` `/model` are the separately-reintroduced backend
    # connect/select commands; true account commands (billing/IDE/voice)
    # stay out of scope for the baseline.
    assert "login" in command_names
    assert "logout" in command_names
    assert "model" in command_names
    refute Enum.any?(command_names, &(&1 in ["billing", "ide", "voice"]))

    assert {:ok, pane} = Registry.fetch(registry, "/pane")
  ...
  test "merged registry preserves plugin source attribution for each accepted plugin command and skill" do
    assert {:ok, plugin_config} =
             ConfigSchema.parse("""
             {
               "plugins": [
                 {
                   "identity": {"id": "ouroboros-plugin", "name": "Ouroboros", "version": "1.2.3"},
                   "path": "plugins/ouroboros",
                   "entrypoint": {"type": "manifest", "path": "capabilities.json"},
                   "enabled": true,
                   "source": "official",
                   ...
                   "provenance": {"publisher": "ourocode", "distribution": "bundled"},
                   "config": {
                     "commands": [
                       {
                         "name": "official-sync",
                         "description": "Run an official sync command.",
                         "action": "sync"
                       }
                     ],
                     "skills": [
                       {
                         "name": "official-audit",
                         "description": "Run an official audit skill.",
                         "mcp_tool": "official_audit"
                       }
                     ]
                   }
                 },
    ...
    plugin_entries =
      ...
      |> Map.new(fn entry -> {entry.slash, entry} end)

    assert Map.keys(plugin_entries) |> Enum.sort() ==
             ["/official-audit", "/official-sync", "/vim-help", "/vim-toggle"]

    assert plugin_entries["/official-sync"].source_attribution.plugin_source == "official"
    assert plugin_entries["/official-sync"].source_attribution.plugin_surface == :command

    assert plugin_entries["/official-sync"].source_attribution.command_namespace ==
             "plugin:official:ouroboros"

    assert plugin_entries["/official-sync"].source_attribution.namespace_owner ==
             :official_ouroboros

    assert plugin_entries["/official-audit"].source_attribution.plugin_source == "official"
    assert plugin_entries["/official-audit"].source_attribution.plugin_surface == :skill

    assert plugin_entries["/official-audit"].source_attribution.command_namespace ==
             "plugin:official:ouroboros"

    assert plugin_entries["/official-audit"].source_attribution.namespace_owner ==
             :official_ouroboros

    assert plugin_entries["/vim-toggle"].source_attribution.plugin_source == "third_party"
    assert plugin_entries["/vim-toggle"].source_attribution.plugin_surface == :command

    assert plugin_entries["/vim-toggle"].source_attribution.command_namespace ==
             "plugin:third_party:vim-mode"

    assert plugin_entries["/vim-toggle"].source_attribution.namespace_owner == :third_party_plugin
    assert plugin_entries["/vim-help"].source_attribution.plugin_source == "third_party"
    assert plugin_entries["/vim-help"].source_attribution.plugin_surface == :skill

    assert plugin_entries["/vim-help"].source_attribution.command_namespace ==
             "plugin:third_party:vim-mode"

    assert plugin_entries["/vim-help"].source_attribution.namespace_owner == :third_party_plugin

    assert Enum.all?(plugin_entries, fn {_slash, entry} ->
             entry.source_attribution.source == :plugin and
               entry.source_attribution.source_id == entry.source_id and
               entry.source_attribution.plugin_id == entry.run_spec.plugin_id and
               entry.source_attribution.loaded_from == entry.run_spec.plugin_path and
               entry.source_attribution.command_namespace == entry.metadata.command_namespace and
               entry.source_attribution.namespace_owner == entry.metadata.namespace_owner and
               entry.metadata.source_attribution == entry.source_attribution
           end)
  end
  ...
  test "MCP command entries expose normalized invocability metadata from tools and annotations" do
    entries =
      Registry.mcp_entries([
        %{
          transport: :stdio,
          server_id: "default-tools",
          tools: [
            %{
              "name" => "default.echo",
              "description" => "Defaults to user-invocable when no MCP hint exists."
            }
          ]
        },
        %{
          transport: :sse,
          server_id: "explicit-tools",
          tools: [
            %{
              "name" => "explicit.run",
              "userInvocable" => "true",
              "description" => "Explicit top-level MCP invocability."
            }
          ]
        },
        %{
          transport: :streamable_http,
          server_id: "annotation-tools",
          tools: [
            %{
              "name" => "annotation.preview",
              "description" => "Annotation-provided MCP invocability.",
              "annotations" => %{"user_invocable" => true}
            },
            %{
              "name" => "annotation.internal",
              "description" => "Filtered internal tool.",
              "annotations" => %{"user_invocable" => false}
            }
          ]
        }
    ...
    entries_by_slash = Map.new(entries, fn entry -> {entry.slash, entry} end)
    default_entry = entries_by_slash["/default-echo"]
    explicit_entry = entries_by_slash["/explicit-run"]
    annotation_entry = entries_by_slash["/annotation-preview"]

    assert default_entry.metadata.invocability == %{
             user_invocable?: true,
             source: :default_user_invocable,
             raw_value: true,
             runnable?: true,
             method: "tools/call"
           }

    assert explicit_entry.metadata.invocability == %{
             user_invocable?: true,
             source: :tool_user_invocable,
             raw_value: "true",
             runnable?: true,
             method: "tools/call"
           }

    assert annotation_entry.metadata.invocability == %{
             user_invocable?: true,
             source: :annotation_user_invocable,
             raw_value: true,
             runnable?: true,
    ...
    assert Enum.all?([default_entry, explicit_entry, annotation_entry], fn entry ->
             entry.runnable? == true and
               entry.availability == :available and
               entry.run_spec.kind == :mcp_tool and
               entry.run_spec.method == "tools/call" and
               entry.run_spec.invocability == entry.metadata.invocability
           end)
  end
  ...
  test "dynamic session skills cannot replace existing commands or aliases" do
    assert {:ok, registry} = Registry.load(bundled_skill_dirs: [], skill_dirs: [])
    ...
    assert {:ok, registry} =
             Registry.add_dynamic_skills(registry, [
               ...
               %{
                 name: "dynamic-review",
                 description: "Accepted dynamic skill.",
                 aliases: ["/cmds"],
                 source_id: "session"
               }
             ])
    ...
    assert Registry.fetch(registry, "/dynamic-review") == :error
    assert Registry.fetch(registry, "/cmds") == Registry.fetch(registry, "/commands")
    assert registry.sources == [:builtin]

    assert Enum.any?(
             registry.duplicates,
             &(&1.source == :dynamic_skill and &1.token == "/help" and
                 &1.winner.source == :builtin)
           )

    assert Enum.any?(
             registry.duplicates,
             &(&1.source == :dynamic_skill and
                 &1.reason == :alias_collision and
                 &1.token == "/cmds" and
                 &1.winner.slash == "/commands")
           )
  end

  test "query APIs retrieve ordered entries and filter the merged command registry" do
    assert {:ok, plugin_config} =
             ConfigSchema.parse("""
             {
               "plugins": [
      ...
      %{
        ...
        tools: [
          %{
            ...
            "description" => "Review through a remote MCP server.",
            "inputSchema" => %{
              "properties" => %{"path" => %{"description" => "Path to review"}}
            }
          }
        ]
      },
    ...
    assert {:ok, registry} = Registry.merge_normalized_entries(registry, stub_entry)

    assert Registry.entries(registry) == registry.ordered
    assert Registry.list(registry) == registry.ordered

    assert Registry.query(registry, source: :builtin, category: :discovery)
           |> Enum.map(& &1.slash) == ["/help", "/commands", "/skills", "/capabilities"]

    assert Registry.query(registry, plugin_id: "vim-mode")
           |> Enum.map(& &1.slash) == ["/vim-help", "/vim-toggle"]

    assert Registry.query(registry, source: :mcp, transport: "streamable-http")
           |> Enum.map(& &1.slash) == ["/remote-review"]

    assert Registry.query(registry, sources: [:mcp], source_id: "agent-tools")
           |> Enum.map(& &1.slash) == ["/agent-summarize"]

    assert Registry.query(registry, prefix: "vim")
           |> Enum.map(& &1.slash) == ["/vim-help", "/vim-toggle"]

    assert Registry.query(registry, prefix: "/vim-m")
           |> Enum.map(& &1.slash) == ["/vim-help"]

    assert Registry.query(registry, text: "child agent")
           |> Enum.map(& &1.slash) == ["/agent-summarize"]

    assert Registry.query(registry, query: "vim_motion_help")
           |> Enum.map(& &1.slash) == ["/vim-help"]

    assert Registry.query(registry, availability: :stub, runnable?: false)
           |> Enum.map(& &1.slash) == ["/billing"]

    assert Registry.query(registry, source: :builtin, limit: 2)
           |> Enum.map(& &1.slash) == ["/help", "/commands"]
  end
