defmodule Ourocode.MCP.Transport.SSETest do
  ...
  test "SSE raw event record builder captures transport endpoint connection session and timestamp" do
    parsed_event = %{
      "event" => "message",
      "id" => "sse-raw-1",
      "data" => %{
    ...
    assert %{
             ...
             transport: :sse,
             transport_type: :sse,
             endpoint_url: "http://127.0.0.1:4001/mcp/events?session=session-sse-raw-1",
             connection_identifier: "sse-connection-raw-1",
             session_identifier: "session-sse-raw-1",
             sse_event_id: "sse-raw-1",
             sse_event_id_present: true,
             sse_event_type: "message",
             sse_event_type_present: true,
             timestamp_ms: 1_714_000_000_123,
             received_at_ms: 1_714_000_000_123,
             raw_payload_ref: "sha256:frame-1"
           } =
             SSE.build_raw_event_record(parsed_event, %{
               endpoint_url: "http://127.0.0.1:4001/mcp/events?session=session-sse-raw-1",
               connection_identifier: "sse-connection-raw-1",
               session_identifier: "session-sse-raw-1",
               sse_event_id: "sse-raw-1",
  ...
  test "SSE raw event record builder stores raw payload out-of-band by stable reference" do
    store_dir =
      Path.join(
        System.tmp_dir!(),
        "ourocode-sse-raw-payloads-#{System.unique_integer([:positive])}"
      )
    ...
    raw_payload = """
    event: child-token
    id: sse-raw-sidecar-1
    data: {"jsonrpc":"2.0","method":"notifications/progress","params":{"token":"secret raw bytes"}}

    """

    expected_ref = "sha256:" <> Base.encode16(:crypto.hash(:sha256, raw_payload), case: :lower)
    expected_size = byte_size(raw_payload)

    debug_record =
      SSE.build_raw_event_record(
        %{
          "event" => "child-token",
          "id" => "sse-raw-sidecar-1",
          "data" => %{
            "jsonrpc" => "2.0",
            "method" => "notifications/progress",
            "params" => %{"token" => "redacted-for-debug-record"}
          },
          frame: raw_payload
        },
        %{
          endpoint_url: "http://127.0.0.1:4001/mcp/events?session=session-sse-sidecar-1",
          connection_identifier: "sse-connection-sidecar-1",
          session_identifier: "session-sse-sidecar-1",
          raw_payload: raw_payload,
          raw_payload_store_dir: store_dir,
          timestamp_ms: 1_714_000_000_789
        }
      )
    ...
    assert %{
             ...
             raw_payload_size_bytes: ^expected_size
           } = debug_record

    refute Map.has_key?(debug_record, :raw_payload)
    refute Map.has_key?(debug_record, :frame)
    refute inspect(debug_record) =~ "secret raw bytes"

    assert {:ok, ^raw_payload} = File.read(SSE.raw_payload_path(store_dir, expected_ref))
  end

  test "SSE raw event record builder preserves explicit absence of event id and type" do
    parsed_event = %{
      "event" => "message",
      "id" => nil,
      "data" => %{
    ...
    assert %{
             ...
             SSE.build_raw_event_record(parsed_event, %{
               endpoint_url: "http://127.0.0.1:4001/mcp/events?session=session-sse-raw-2",
               connection_identifier: "sse-connection-raw-2",
               session_identifier: "session-sse-raw-2",
               sse_event_id: nil,
               sse_event_id_present: false,
               sse_event_type: nil,
               sse_event_type_present: false,
               timestamp_ms: 1_714_000_000_456
             })
  end
  ...
  test "incoming SSE parent MCP call events become UI hierarchy root nodes" do
    {:ok, server} =
      start_fake_sse_server([
        {:send,
         [
    ...
    assert {:ok, _transport} =
             SSE.start_link(
               url: "http://127.0.0.1:#{server.port}/mcp/events?session=session-sse-root-1",
               parent_call_id: "parent-sse-root-1",
               runtime_source: "synthetic",
               external_ids: %{
                 "session_id" => "session-sse-root-1",
                 "call_id" => "call-sse-root-1"
               },
               event_sink: self(),
               timeout: 1_000
             )
    ...
    assert_receive {:fake_sse_request, request}, 500
    assert request =~ "GET /mcp/events?session=session-sse-root-1 HTTP/1.1"
    ...
    {parent_state, child_state} =
      {
        ParentMcpPane.apply_event(%{working: [], completed: [], focused: nil, open: []}, event),
    ...
    assert %{
             ...
             roots: [
               %{
                 id: "parent-mcp:parent-sse-root-1",
                 kind: :parent_mcp_call,
                 status: "streaming",
                 parent_call_id: "parent-sse-root-1",
                 runtime_source: "synthetic",
                 transport: "sse",
                 child_id: "child-sse-root-1",
                 stream_cursor: %{
                   transport: :sse,
                   parent_call_id: "parent-sse-root-1",
                   event_seq: 2
                 ...
                 children: [
                   %{
                     id: "child-session:child-sse-root-1",
                     kind: :child_session,
                     status: "working",
                     child_id: "child-sse-root-1",
                     parent_call_id: "parent-sse-root-1",
  ...
  test "attaches SSE child session events under the matching parent MCP call node" do
    {:ok, server} =
      start_fake_sse_server([
        {:send,
         [
    ...
    assert {:ok, _transport} =
             SSE.start_link(
               url: "http://127.0.0.1:#{server.port}/mcp/events?session=session-sse-attach-1",
               parent_call_id: "parent-sse-attach-1",
               runtime_source: "synthetic",
               external_ids: %{
                 "session_id" => "session-sse-attach-1",
                 "call_id" => "call-sse-attach-1"
               },
               event_sink: self(),
               timeout: 1_000
             )
    ...
    assert_receive {:fake_sse_request, request}, 500
    assert request =~ "GET /mcp/events?session=session-sse-attach-1 HTTP/1.1"
    ...
    unrelated_parent_event = %LifecycleEvent{
      type: :parent_call_started,
      transport: :sse,
      parent_call_id: "parent-sse-unrelated",
      runtime_source: "synthetic",
      external_ids: %{"session_id" => "session-sse-unrelated"},
      request_id: "call-sse-unrelated",
      method: "tools/call",
      params: %{"name" => "ooo.run"},
      event_seq: 1,
      occurred_at_ms: 1
    ...
    {parent_state, child_state} =
      Enum.reduce(
        [unrelated_parent_event, sse_child_event],
        {
          %{working: [], completed: [], focused: nil, open: []},
          %{working: [], completed: [], focused: nil, open: []}
        },
    ...
    assert [
             ...
             %{
               id: "parent-mcp:parent-sse-attach-1",
               kind: :parent_mcp_call,
               status: "streaming",
               parent_call_id: "parent-sse-attach-1",
               transport: "sse",
               child_id: "child-sse-attach-1",
               children: [
                 %{
                   id: "child-session:child-sse-attach-1",
                   kind: :child_session,
                   status: "working",
                   child_id: "child-sse-attach-1",
                   parent_call_id: "parent-sse-attach-1",
  ...
  test "transport reconstructs arbitrarily chunked SSE byte streams without loss" do
    ...
    child_state =
      Enum.reduce(stream_events ++ [result_event], ChildSessionPanes.new(), fn event, state ->
        ChildSessionPanes.apply_event(state, event)
      end)
    ...
    assert [
             %{
               ...
               parent_call_id: "parent-chunked-sse-1",
               transport: :sse,
               stream_cursor: %{event_seq: ^result_event_seq},
               pane_state: %{stream_entries: stream_entries}
             }
           ] = child_state.working

    assert Enum.map(stream_entries, & &1.runtime_seq) == Enum.to_list(1..(event_count + 1))

    assert Enum.map(stream_entries, & &1.token) ==
             Enum.map(1..event_count, &"chunked-sse-token-#{&1}") ++ ["chunked-sse-done"]

    assert {:ok, journal_entries} = Journal.read_ordered(journal_path)
    relevant_journal_entries = Enum.take(journal_entries, event_count + 2)

    assert Enum.map(relevant_journal_entries, & &1.event_seq) ==
             Enum.to_list(1..(event_count + 2))

    assert Enum.map(relevant_journal_entries, & &1.type) ==
             [:transport_connected] ++
               List.duplicate(:parent_call_event, event_count) ++ [:parent_call_result]

    assert Enum.map(Enum.slice(relevant_journal_entries, 1, event_count), fn entry ->
             get_in(entry, [:notification, "params", "seq"])
           end) == Enum.to_list(1..event_count)

    assert List.last(relevant_journal_entries).result["seq"] == event_count + 1
  end
  ...
  test "derives deterministic fallback runtime IDs from SSE event metadata when childID is absent" do
    {:ok, server} =
      start_fake_sse_server([
        {:send,
         [
         ...
         sse_frame(
           %{
             ...
             "params" => %{
               "seq" => 1,
               "token" => "hello",
               "threadId" => "thread-sse-fallback-1",
               "metadata" => %{"sessionId" => "session-sse-fallback-1"}
             }
           },
           id: "sse-frame-1"
         )},
    ...
    assert {:ok, _transport} =
             SSE.start_link(
               ...
               parent_call_id: "parent-sse-fallback",
    ...
    assert_receive {:ourocode_event,
                    %LifecycleEvent{
                      ...
                      runtime_source: "codex",
                      external_ids: %{
                        thread_id: "thread-sse-fallback-1",
                        session_id: "session-sse-fallback-1"
                      },
                      notification: %{
                        "params" => %{
                          "seq" => 1,
    ...
    fallback_id = "fallback:thread_id:thread-sse-fallback-1"

    assert %{
             working: [
               %{
                 child_id: ^fallback_id,
                 stream_cursor: %{transport: :sse, child_id: ^fallback_id},
                 external_ids: %{
                   :thread_id => "thread-sse-fallback-1",
                   :session_id => "session-sse-fallback-1",
  ...
  test "maps repeated SSE fallback events to one stable child pane identity" do
    {:ok, server} =
      start_fake_sse_server([
        {:send,
         [
        ...
        {:send,
         sse_frame(%{
           ...
           "params" => %{
             "seq" => 1,
             "token" => "first",
             "metadata" => %{"sessionId" => "session-sse-stable-1"}
           }
         })},
        {:sleep, 25},
        {:send,
    ...
    assert {:ok, _transport} =
             SSE.start_link(
               url: "http://127.0.0.1:#{server.port}/mcp/events",
               parent_call_id: "parent-sse-stable",
               runtime_source: "codex",
               external_ids: %{},
               event_sink: self(),
               timeout: 1_000
    ...
    assert_receive {:ourocode_event,
                    %LifecycleEvent{
                      type: :parent_call_event,
                      event_seq: 2,
                      external_ids: %{session_id: "session-sse-stable-1"}
                    } = first_event},
                   500
    ...
    assert_receive {:ourocode_event,
                    %LifecycleEvent{
                      ...
                      event_seq: 3,
                      external_ids: %{
                        session_id: "session-sse-stable-1",
                        thread_id: "thread-sse-stable-1"
                      }
                    } = second_event},
                   500

    fallback_id = "fallback:session_id:session-sse-stable-1"
    pane_id = "child-session:" <> fallback_id

    assert %{
             child_pane_registry: %{^fallback_id => ^pane_id},
             open: [^pane_id],
             working: [
               %{
                 id: ^pane_id,
                 child_id: ^fallback_id,
                 stream_cursor: %{transport: :sse, event_seq: 3, child_id: ^fallback_id},
                 external_ids: %{
                   :session_id => "session-sse-stable-1",
                   :thread_id => "thread-sse-stable-1",
                   "fallback_child_id" => ^fallback_id,
             ...
             %{working: [], completed: [], focused: nil, open: []}
             |> ChildSessionPanes.apply_event(first_event)
             |> ChildSessionPanes.apply_event(second_event)
  end
