defmodule Ourocode.MCP.Transport.StreamableHTTPTest do
  ...
  test "renders a streamable HTTP child session nested under its parent MCP call" do
    {:ok, server} =
      start_fake_http_mcp_server(
        %{
          childID: "child-http-ui-1",
          seq: 1,
          token: "first",
          ok: true
        },
    ...
    request = %{
      ...
      id: "call-http-ui-1",
      method: "tools/call",
      params: %{name: "ooo.run", arguments: %{task: "inspect streamable HTTP hierarchy"}}
    }
    ...
    assert {:ok, %ParentCallResult{response: response}} =
             StreamableHTTP.execute_parent_call(
               [
                 url: "http://127.0.0.1:#{server.port}/mcp",
                 parent_call_id: "parent-http-ui-1",
                 runtime_source: "synthetic",
                 external_ids: %{
                   "session_id" => "session-http-ui-1",
                   "call_id" => "call-http-ui-1"
                 },
                 subscriber: self(),
                 timeout: 2_000
               ],
    ...
    assert [
             %{
               id: "parent-mcp:parent-http-ui-1",
               ...
               children: [
                 %{
                   id: "child-session:child-http-ui-1",
                   kind: :child_session,
                   status: "working",
                   child_id: "child-http-ui-1",
                   parent_call_id: "parent-http-ui-1",
                   runtime_source: "synthetic",
                   transport: "streamable_http",
                   stream_cursor: %{
    ...
    assert_receive {:fake_server_request, raw_request}, 500
    assert raw_request =~ "POST /mcp HTTP/1.1"
    assert raw_request =~ "\"task\":\"inspect streamable HTTP hierarchy\""
  end
  ...
  test "routes the first streamable HTTP token for an existing childID to its child pane" do
    ...
    tree = UITree.from_panes(parent_state, final_child_state)

    assert [
             %{
               id: "parent-mcp:parent-http-existing-1",
               children: [
                 %{
                   id: "child-pane:stable-existing-http",
                   child_id: "child-http-existing-1",
                   stream_events: [
                     %{event_seq: 2, runtime_seq: 1, token: "first-existing-token"},
  ...
  test "normalizes streamable HTTP SSE parent lifecycle events into typed events" do
    ...
    assert {:ok,
            ...
            ]} = LifecycleNormalizer.normalize_body(200, headers, body, context)
  end

  test "normalizes streamable HTTP lifecycle response chunks into shared event schema" do
    headers = [{"content-type", "text/event-stream"}]
  ...
  test "normalizes streamable HTTP event shapes into the canonical UI tree model" do
    headers = [{"content-type", "text/event-stream"}]

    body = """
    event: message
    data: {"jsonrpc":"2.0","method":"notifications/progress","params":{"childID":"child-http-tree-1","seq":1,"token":"hello","cursor":{"offset":10}}}

    event: message
    data: {"jsonrpc":"2.0","method":"notifications/progress","params":{"childID":"child-http-tree-1","seq":2,"delta":" world","cursor":{"offset":20}}}

    event: message
    data: {"jsonrpc":"2.0","id":"call-http-tree-1","result":{"ok":true,"childID":"child-http-tree-1","seq":3,"content":"done"}}

    """

    context = %{
      event_seq: 2,
      parent_call_id: "parent-http-tree-1",
      runtime_source: "synthetic",
      external_ids: %{"session_id" => "session-http-tree-1"},
      request_id: "call-http-tree-1",
      method: "tools/call",
      params: %{"name" => "synthetic.tree"},
      occurred_at_ms: 123
    }

    started = LifecycleNormalizer.started(%{context | event_seq: 1})
    ...
    assert %{
             ...
             roots: [
               %{
                 id: "parent-mcp:parent-http-tree-1",
                 kind: :parent_call,
                 status: :completed,
                 parent_call_id: "parent-http-tree-1",
                 runtime_source: "synthetic",
                 ...
                 method: "tools/call",
                 external_ids: %{
                   "session_id" => "session-http-tree-1",
                   childID: "child-http-tree-1"
                 },
                 stream_cursor: %{
                   transport: :streamable_http,
                   parent_call_id: "parent-http-tree-1",
                 ...
                 children: [
                   %{
                     id: "child-session:child-http-tree-1",
                     kind: :child_session,
                     status: :working,
                     child_id: "child-http-tree-1",
                     parent_call_id: "parent-http-tree-1",
                     runtime_source: "synthetic",
                     transport: :streamable_http,
                     stream_cursor: %{
