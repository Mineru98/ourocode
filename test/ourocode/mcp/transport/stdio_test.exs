defmodule Ourocode.MCP.Transport.StdioTest do
  ...
  test "parses stdio stdout lifecycle output into typed lifecycle records" do
    command = System.find_executable("sh")
    assert is_binary(command)

    script = """
    while IFS= read -r line; do
      printf '%s\n' 'plain helper startup line'
      printf '%s\n' '{"jsonrpc":"2.0","method":"notifications/progress","params":{"childID":"child-typed-stdio-1","seq":1,"token":"typed-token"}}'
      printf '%s\n' '{"jsonrpc":"2.0","id":"1","result":{"ok":true,"childID":"child-typed-stdio-1","seq":2}}'
    done
    """
    ...
    {:ok, transport} =
      Stdio.start_link(
        command: command,
        args: ["-c", script],
        parent_call_id: "parent-typed-stdio-1",
        runtime_source: "synthetic",
        external_ids: %{"session_id" => "session-typed-stdio-1"},
        event_sink: self()
      )
    ...
    assert {:ok, %{"ok" => true, "childID" => "child-typed-stdio-1", "seq" => 2}} =
             Stdio.call_parent(transport, "tools/call", %{"name" => "synthetic.typed"},
               timeout: 1_000
             )
    ...
    assert_receive {:ourocode_event,
                    %LifecycleEvent{
                      ...
                      raw_event: %{"params" => %{"childID" => "child-typed-stdio-1"}},
                      event_seq: 3
                    }}
    ...
    assert_receive {:ourocode_event,
                    %LifecycleEvent{
                      ...
                      request_id: "1",
                      method: "tools/call",
                      payload: %{"ok" => true, "childID" => "child-typed-stdio-1", "seq" => 2},
                      result: %{"ok" => true, "childID" => "child-typed-stdio-1", "seq" => 2},
                      raw_event: %{"id" => "1", "result" => %{"childID" => "child-typed-stdio-1"}},
                      event_seq: 4
                    }}
  end
  ...
  test "nests stdio child session panes under the matching parent MCP call without sibling leakage" do
    command = System.find_executable("sh")
    assert is_binary(command)

    child_script = fn child_id, token ->
      """
      while IFS= read -r line; do
        printf '%s\n' '{"jsonrpc":"2.0","method":"notifications/progress","params":{"childID":"#{child_id}","seq":1,"token":"#{token}"}}'
        printf '%s\n' '{"jsonrpc":"2.0","id":"1","result":{"ok":true,"childID":"#{child_id}","seq":2}}'
      done
      """
    end

    {:ok, parent_a_transport} =
      Stdio.start_link(
        command: command,
        args: ["-c", child_script.("child-stdio-a", "alpha")],
        parent_call_id: "parent-stdio-a",
        runtime_source: "synthetic",
        external_ids: %{"session_id" => "session-stdio-a"},
        event_sink: self()
      )

    {:ok, parent_b_transport} =
      Stdio.start_link(
        command: command,
        args: ["-c", child_script.("child-stdio-b", "bravo")],
        parent_call_id: "parent-stdio-b",
        runtime_source: "synthetic",
        external_ids: %{"session_id" => "session-stdio-b"},
        event_sink: self()
      )

    assert_receive {:ourocode_event,
                    %{type: :transport_started, parent_call_id: "parent-stdio-a"}}

    assert_receive {:ourocode_event,
                    %{type: :transport_started, parent_call_id: "parent-stdio-b"}}

    assert {:ok, %{"ok" => true, "childID" => "child-stdio-a", "seq" => 2}} =
             Stdio.call_parent(parent_a_transport, "tools/call", %{"name" => "parent.a"},
               timeout: 1_000
             )

    assert_receive {:ourocode_event,
                    %{type: :parent_call_started, parent_call_id: "parent-stdio-a"} = a_started}

    assert_receive {:ourocode_event,
                    %{type: :parent_call_event, parent_call_id: "parent-stdio-a"} = a_event}

    assert_receive {:ourocode_event,
                    %{type: :parent_call_result, parent_call_id: "parent-stdio-a"} = a_result}

    assert {:ok, %{"ok" => true, "childID" => "child-stdio-b", "seq" => 2}} =
             Stdio.call_parent(parent_b_transport, "tools/call", %{"name" => "parent.b"},
               timeout: 1_000
             )

    assert_receive {:ourocode_event,
                    %{type: :parent_call_started, parent_call_id: "parent-stdio-b"} = b_started}

    assert_receive {:ourocode_event,
                    %{type: :parent_call_event, parent_call_id: "parent-stdio-b"} = b_event}

    assert_receive {:ourocode_event,
                    %{type: :parent_call_result, parent_call_id: "parent-stdio-b"} = b_result}

    {parent_state, child_state} =
      Enum.reduce(
        [a_started, a_event, a_result, b_started, b_event, b_result],
        {
          %{working: [], completed: [], focused: nil, open: []},
          %{working: [], completed: [], focused: nil, open: []}
        },
    ...
    hierarchy = Layout.parent_child_hierarchy(parent_state, child_state)
    roots_by_parent = Map.new(hierarchy.roots, &{&1.parent_call_id, &1})

    assert hierarchy.orphan_children == []
    assert Map.keys(roots_by_parent) |> Enum.sort() == ["parent-stdio-a", "parent-stdio-b"]

    assert [%{id: "child-session:child-stdio-a", child_id: "child-stdio-a"}] =
             roots_by_parent["parent-stdio-a"].children

    assert [%{id: "child-session:child-stdio-b", child_id: "child-stdio-b"}] =
             roots_by_parent["parent-stdio-b"].children

    refute Enum.any?(
             roots_by_parent["parent-stdio-a"].children,
             &(&1.child_id == "child-stdio-b")
           )

    refute Enum.any?(
             roots_by_parent["parent-stdio-b"].children,
             &(&1.child_id == "child-stdio-a")
           )
  end

  test "normalizes stdio JSONL event shapes into the canonical UI tree model" do
    command = System.find_executable("sh")
    assert is_binary(command)

    script = """
    while IFS= read -r line; do
      printf '%s\n' '{"jsonrpc":"2.0","method":"notifications/progress","params":{"childID":"child-stdio-tree-1","seq":1,"token":"hello","cursor":{"offset":10}}}'
      printf '%s\n' '{"jsonrpc":"2.0","method":"notifications/progress","params":{"childID":"child-stdio-tree-1","seq":2,"delta":" world","cursor":{"offset":20}}}'
      printf '%s\n' '{"jsonrpc":"2.0","id":"1","result":{"ok":true,"childID":"child-stdio-tree-1","seq":3,"content":"done"}}'
    done
    """
    ...
    {:ok, transport} =
      Stdio.start_link(
        command: command,
        args: ["-c", script],
        parent_call_id: "parent-stdio-tree-1",
        runtime_source: "synthetic",
        external_ids: %{"session_id" => "session-stdio-tree-1"},
        event_sink: self()
      )

    assert_receive {:ourocode_event,
                    %{type: :transport_started, parent_call_id: "parent-stdio-tree-1"}}

    assert {:ok, %{"ok" => true, "childID" => "child-stdio-tree-1", "seq" => 3}} =
             Stdio.call_parent(transport, "tools/call", %{"name" => "synthetic.tree"},
               timeout: 1_000
             )
    ...
    assert_receive {:ourocode_event,
                    %{
                      ...
                      parent_call_id: "parent-stdio-tree-1",
                      notification: %{
                        "params" => %{"childID" => "child-stdio-tree-1", "seq" => 1}
                      },
                      raw_event: %{"params" => %{"childID" => "child-stdio-tree-1", "seq" => 1}},
                      event_seq: 3
                    } = first}
    ...
    assert_receive {:ourocode_event,
                    %{
                      ...
                      parent_call_id: "parent-stdio-tree-1",
                      notification: %{
                        "params" => %{"childID" => "child-stdio-tree-1", "seq" => 2}
                      },
                      raw_event: %{"params" => %{"childID" => "child-stdio-tree-1", "seq" => 2}},
                      event_seq: 4
                    } = second}
    ...
    assert_receive {:ourocode_event,
                    %{
                      ...
                      parent_call_id: "parent-stdio-tree-1",
                      request_id: "1",
                      raw_event: %{"id" => "1", "result" => %{"childID" => "child-stdio-tree-1"}},
                      event_seq: 5
                    } = result}

    tree = UITree.from_events([started, first, second, result])
    ...
    assert %{
             ...
             roots: [
               %{
                 id: "parent-mcp:parent-stdio-tree-1",
                 kind: :parent_call,
                 status: :completed,
                 parent_call_id: "parent-stdio-tree-1",
                 runtime_source: "synthetic",
                 ...
                 method: "tools/call",
                 external_ids: %{
                   "session_id" => "session-stdio-tree-1",
                   childID: "child-stdio-tree-1"
                 },
                 stream_cursor: %{
                   transport: :stdio,
                   parent_call_id: "parent-stdio-tree-1",
                 ...
                 children: [
                   %{
                     id: "child-session:child-stdio-tree-1",
                     kind: :child_session,
                     status: :working,
                     child_id: "child-stdio-tree-1",
                     parent_call_id: "parent-stdio-tree-1",
                     runtime_source: "synthetic",
                     transport: :stdio,
                     stream_cursor: %{
